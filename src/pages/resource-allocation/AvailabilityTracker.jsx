...
import { Calendar, Search, Download, Filter, UserPlus, ArrowRight, CheckCircle2, Plus, X } from 'lucide-react';
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-800">
              {loading ? (
                <tr>
                  <td colSpan="5" className="px-6 py-12 text-center text-slate-500 font-bold tracking-widest uppercase text-[10px]">Loading availability...</td>
                </tr>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <div className="w-24 bg-slate-800 h-1.5 rounded-full overflow-hidden">
                          <div 
                            className={`h-full rounded-full ${res.availabilityPercentage > 50 ? 'bg-emerald-500' : 'bg-blue-500'}`}
                            style={{ width: `${res.availabilityPercentage}%` }}
                          />
                        </div>
                        <span className="text-xs font-bold text-slate-400">{res.availabilityPercentage}%</span>
                      </div>
                    </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-slate-900 border border-slate-800 rounded-[2rem] w-full max-w-xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
            <div className="p-8 border-b border-slate-800 flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-black text-white tracking-tight">Add New Resource</h2>
                <p className="text-slate-400 text-xs font-bold mt-1 uppercase tracking-widest">Register resource for tracking</p>
              </div>
              <button 
